            ...
            <div class="card card-profile shadow">
                <div class="card-header text-center border-0 pt-4 pt-md-4 pb-0 pb-md-4">
                    <div class="d-flex justify-content-between">
                        <a href="{{ route('cadastros.create') }}" class="btn btn-sm btn-info mr-4">{{ __('Atualizar Dados Cadastrais') }}</a>
                        <a href="{{ route('profile.edit') }}" class="btn btn-sm btn-default float-right">{{ __('Atualizar Login') }}</a>
                    </div>
                </div>
                <br>                
                <div class="card-body pt-0 pt-md-4">
                    <form method="post" action="{{ route('bancarios.store') }}" autocomplete="off">
                        @csrf
                        ...
                        <div class="pl-lg-4">
                            <div class="form-group{{ $errors->has('banco') ? ' has-danger' : '' }}">
                                ...
                                <input type="text" name="banco" id="input-banco" class="form-control form-control-alternative{{ $errors->has('banco') ? ' is-invalid' : '' }}" placeholder="{{ __('Banco') }}" value="{{ old('banco') }}" required autofocus>
                            ...
                            <div class="form-group{{ $errors->has('agencia') ? ' has-danger' : '' }}">
                                ...
                                <input type="number" name="agencia" id="input-agencia" class="form-control form-control-alternative{{ $errors->has('agencia') ? ' is-invalid' : '' }}" placeholder="{{ __('Agência') }}" value="{{ old('agencia') }}" required>
                            ...
                            <div class="form-group{{ $errors->has('conta_corrente') ? ' has-danger' : '' }}">
                                ...
                                <input type="number" name="conta_corrente" id="input-conta_corrente" class="form-control form-control-alternative{{ $errors->has('conta_corrente') ? ' is-invalid' : '' }}" placeholder="{{ __('Conta Corrente') }}" value="{{ old('conta_corrente') }}" required>
